...
/**
 * Resolve API key from command line args or environment variables
 * Returns API key or null if not configured
 */
export function resolveApiKey(): { key: string; source: string } | null {
  // Get command line arguments
  const args = parseCommandLineArgs();

  // 1. Check command line arguments first (highest priority)
  if (args["api-key"]) {
    return { key: args["api-key"], source: "command line argument" };
  }

  // 2. Check environment variables
  if (process.env.API_KEY) {
    return { key: process.env.API_KEY, source: "environment variable (API_KEY)" };
  }

  // 3. Check for deprecated MCP_API_KEY
  if (process.env.MCP_API_KEY) {
    return { key: process.env.MCP_API_KEY, source: "environment variable (MCP_API_KEY)" };
  }
