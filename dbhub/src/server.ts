...
import { resolveTransport, resolvePort, resolveSourceConfigs, isDemoMode, resolveApiKey } from "./config/env.js";
...
`);
    ...
    // Resolve API key if configured
      const apiKeyConfig = resolveApiKey();
      
      // Log startup configuration
      if (apiKeyConfig) {
        console.error(`API key authentication enabled (from ${apiKeyConfig.source})`);
      } else {
        console.error(`⚠ WARNING: API key authentication is DISABLED`);
        console.error(`  To enable, set API_KEY environment variable or use --api-key command-line argument`);
      }
      ...
      // Data sources API endpoints (no auth required for now)
      ...
      const authenticateMcp = (req: express.Request, res: express.Response, next: express.NextFunction) => {
        // If no API key is configured, REJECT all requests with 401
        if (!apiKeyConfig) {
          return res.status(401).json({
            error: 'Unauthorized',
            message: 'API key authentication is not configured on the server. Contact your administrator.'
          });
        }
        ...
        // 1. Check Authorization header (supports multiple formats)
        const authHeader = req.headers.authorization;
        if (authHeader) {
          // Try format: "Bearer <key>"
          if (authHeader.startsWith('Bearer ')) {
            providedKey = authHeader.substring(7).trim();
          }
          // Try format: "ApiKey <key>" or "Token <key>"
          ...
          // Try raw key (no scheme)
          else if (authHeader.trim().length > 0) {
            providedKey = authHeader.trim();
          }
        }

        // 2. Check custom header: X-API-Key (if not found in Authorization)
        if (!providedKey && req.headers['x-api-key']) {
          providedKey = (req.headers['x-api-key'] as string).trim();
        }

        // 3. Check query parameter: ?api_key=<key> (if not found elsewhere)
        if (!providedKey && req.query.api_key) {
          providedKey = (req.query.api_key as string).trim();
        }
        ...
        if (!providedKey) {
          return res.status(401).json({
            error: 'Unauthorized',
            message: 'API key is required. Provide it via: Authorization header (Bearer/ApiKey/Token <key>), X-API-Key header, or ?api_key=<key> query param'
          });
        }

        if (providedKey !== apiKeyConfig.key) {
          console.error(`Authentication failure: Invalid API key provided`);
      ...
      app.listen(port, '0.0.0.0', () => {
        ...
        // Show API key authentication status
        if (apiKeyConfig) {
          console.error(`✓ API key authentication enabled`);
          console.error(`  Provide key via: Authorization header (Bearer <key>), ?api_key=<key>, or X-API-Key header`);
        } else {
          console.error(`⚠ API key authentication disabled - set API_KEY environment variable to enable`);
        }
      });
    } else {
